/*
  ...
  NeoPixel DataIn: D4 with 220r series resistor
    ...
    Version 1.4    : Add Configuration_Settings.h tab
...
String inputString = "";  //TODO remove
...
typedef struct Packet
{
    ...
    char cpuName[STRING_SIZE];
    float cpuTemp;
    float cpuLoad;
    float cpuClock;

    char gpuName[STRING_SIZE];
    float gpuTemp;
    float gpuLoad;
    float gpuCoreClock;
    float gpuMemoryClock;
    float gpuShaderClock;
...
void loop() {
  ...
  if (displayChangeDelay > 0 && ((millis() - lastDisplayChange) > displayChangeDelay))
  {
    displayChangeMode = (displayChangeMode + 1) % DISPLAY_COUNT;
  ...
  if (stringComplete) {
    ...
    if (displayChangeMode == 0) {
      DisplayStyle1();
    } else if (displayChangeMode == 1) {
      DisplayStyle2();
...
void send (const Packet* table)
{
    Serial.write((const char*)table, sizeof(Packet));  // 2 bytes.
}

bool receive(Packet* table)
{
    return (Serial.readBytes((char*)table, sizeof(Packet)) == sizeof(Packet));
}
...
char* removeWordsFromLeft(char* string, int words) {
  int countWords = 0;
